...
DOS20192018_DDOS_FLOWS = {'attackers': ['18.218.115.60', '18.219.9.1','18.219.32.43','18.218.55.126','52.14.136.135','18.219.5.43','18.216.200.189','18.218.229.235','18.218.11.51','18.216.24.42'],
                      'victims': ['18.218.83.150','172.31.69.28']}

DOS20192017_DDOS_FLOWS = {'attackers': ['172.16.0.1'],
...
DDOS_ATTACK_SPECS = {
    'DOS2017' : DOS20192017_DDOS_FLOWS,
    'DOS2018' : DOS20192018_DDOS_FLOWS,
...
def apply_labels(flows, labelled_flows, labels, traffic_type):
    for five_tuple, flow in flows.items():
        if labels is not None:
            short_key = (five_tuple[0], five_tuple[2])  # for DOS20192017/DOS20192018 dataset the labels have shorter keys
...
def main(argv):
    ...
    if args.dataset_folder is not None and args.dataset_type is not None:
        ...
        filelist = glob.glob(args.dataset_folder[0]+ '/SAT-*')
    ...
    if args.balance_folder is not None and args.output_folder is not None:
        ...
        for file in datasets:
            ...
            if 'train' in filename:
                ...
                if output_filename_prefix ==None:
                    output_filename_prefix = filename.split('DOS2019')[0].strip()
                else:
                    if filename.split('DOS2019')[0].strip() != output_filename_prefix:
        ...
        for key,value in final_X.items():
            filename = output_filename_prefix + 'DOS2019201X-dataset-balanced-' + key + '.hdf5'
